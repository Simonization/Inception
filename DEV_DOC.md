...
Two files must be changed — both the Docker port mapping and the NGINX listen directive must match.
...
2. In `srcs/requirements/nginx/conf/nginx.conf`, change the listen directive:
   ```nginx
...
4. Clear browser cache (Ctrl+Shift+Delete → Cookies + Cache → Clear)

5. Visit `https://slangero.42.fr:8443`

> **Note:** Both the container port and the NGINX listen port must match. If you only change the docker-compose mapping without updating `nginx.conf`, NGINX still listens on 443 inside the container while Docker forwards to 8443 — the connection will fail.
